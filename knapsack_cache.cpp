...
inline int rli(int w, int j, int N) {return j+w*(N);}
...
void rbacktrack_implicit(vector<int>&K, vector<int>&M, const vector<int>& wts, const vector<int>& vals, int W, 
                        int first, int last, vector<int>& path, int N);
int rfindk(vector<int>&K, vector<int>&M, const vector<int>& wts, const vector<int>& vals, int W, 
...
int main(int argc, char* argv[]) {
    ...
    rbacktrack_implicit(table3, midpts, weight, value, capacity, 0, nitems-1, path, nitems);
...
void rbacktrack_implicit(vector<int>&K, vector<int>&M, const vector<int>& wts, const vector<int>& vals, int W, 
    ...
    // recursive case
    int mid = (first+last) / 2;
    int k = rfindk(K,M,wts,vals,W,first,last, N);
    // set middle element of path
    if(first) {
        // need to offset based on capacity already used
        path[mid] = k + path[first-1]; 
    ...
    rbacktrack_implicit(K,M,wts,vals,k,first,mid,path,mid-first);
    rbacktrack_implicit(K,M,wts,vals,W-k,mid+1,last,path,N-(mid-first));
...
int rfindk(vector<int>&K, vector<int>&M, const vector<int>& wts, const vector<int>& vals, int W, 
    ...
    for(j = first+1; j <= last; j++) {
        ...
        for(w = 0; w <= W; w++) {
            ...
            // if item doesn't fit or not including it is better
            if(w < wts[j] || K[rli(w,prev,2)] >= vals[j] + K[rli(w-wts[j],prev,2)]) {
                K[rli(w,curr,2)] = K[rli(w,prev,2)];
                // update midpoint value
                if(j > mid) {
                    M[rli(w,curr,2)] = M[rli(w,prev,2)];
                }
            }
            // else including it is better
            else {
                K[rli(w,curr,2)] = vals[j] + K[rli(w-wts[j],prev,2)];
                // update midpt value
                if(j > mid) {
                    M[rli(w,curr,2)] = M[rli(w-wts[j],prev,2)];
                }
            }
        }
    }
